...
public class TrackedImageInfo : MonoBehaviour
{
    ...
    // NOTE: as of AR Foundation 6.0, the old trackedImagesChanged
    // event (and its ARTrackedImagesChangedEventArgs type) is
    // deprecated in favor of the generic trackablesChanged event,
    // which now uses ARTrackablesChangedEventArgs<ARTrackedImage>.
    // The added/updated/removed lists work the same way as before,
    // just through this new event instead.
    ...
    void OnChanged(ARTrackablesChangedEventArgs<ARTrackedImage> eventArgs)
    {
        ...
        foreach (var removedImage in eventArgs.removed)
            RemoveMarker(removedImage.Value);
    }
    ...
    void UpdateMarker(ARTrackedImage updatedImage)
    {
        ...
        {
            // If the object doesn't exist yet, spawn it now.
            // ARCore sometimes skips the added event for previously
            // seen markers and fires updated directly instead, so this
            // fallback ensures objects always get created regardless.
            if (name.StartsWith("node_") && !spawnedNodes.ContainsKey(name))
                HandleNewMarker(updatedImage);
            else if (name.StartsWith("tail_") && !spawnedTails.ContainsKey(name))
                HandleNewMarker(updatedImage);
            else if (name.StartsWith("head_") && !spawnedHeads.ContainsKey(name))
                HandleNewMarker(updatedImage);
